# V2 Implementation Plan â€” Prenatal Ultrasound Application

**Based on:** `docs2/refactoring-findings.md` (all sections including Â§12, Â§13, Â§14, Â§15)  
**Scope:** Full-stack refactoring sprint â€” data model, architecture, UI, PDF, migration  
**Structure:** Phase 1 (Greenfield Development) â†’ Phase 2 (Migration Procedures)

> **Layout ruling:** Â§13.3 is authoritative for the N > 2 fetus screen layout (horizontal-scroll flex). The tabs pattern shown in Â§15.5.3 is disregarded.
>
...
> - **Form inputs (`ObservableRow`):** suffix appended to the field *label* â€” `"BPD GA (Hadlock)"`, `"BPD GA (manual)"`, etc.
> - **Detail page value cells (`ExaminationSections`, `ExaminationDetailPage`):** suffix appended to the rendered *value* â€” `"28w 2d (Hadlock)"`, `"28w 2d (manual)"`, etc.
>
> In both contexts: `(manual)` is rendered in yellow `#f1c21b` via a `<span>` wrapper; `(Hadlock)` / `(auto)` / `(Robinson)` are rendered in normal text colour. Footnote rows that previously displayed *"â— Value manually entered"* are removed â€” the suffix in the cell/label is self-explanatory. `autoSuffix` is extracted as a shared utility in `AutoCalcHelpers.tsx`, replacing `autoCalcLabel`. `AutoCalcDot` remains in the codebase but is no longer called anywhere in the form or detail page.
>
> **PDF dagger ruling:** `withManualMarker()` and the ` â€ ` dagger character on manual values in `viewModelBuilders.ts` are **fully preserved**. The dagger footnote legend (`\nâ€  Value manually entered`) is **always statically appended** to the PDF notes string alongside the biometric citations, without conditionally scanning `isManual` flags across fetuses.
...
| R-10 | `primaryRowKey` new field on lookup entity | Advisory | ST-02 stores `primaryRowKey` on the EXAM lookup entity to enable O(1) update (Â§4.3). This is a new field not currently in the `Examination` interface. It must be added to `api/src/types/index.ts` as optional: `primaryRowKey?: string`. |
| R-11 | Biometric citation footnote in new view model | Advisory | **Simplified:** `viewModelBuilders.ts` will **always statically append** `\nâ€  Value manually entered` to the notes citation string unconditionally (no dynamic scanning of `isManual` flags across `fetuses[i]`). Individual measurement fields still use `withManualMarker(val, isManual)` to place the ` â€ ` symbol when manually entered. |
| R-12 | ST-10 (directory restructuring) is a large git operation | Advisory | Moving 28+ files with `git mv` and updating all import paths has a high risk of introducing merge conflicts if any other branch is active. ST-10 should be committed atomically with an automated import-path rewrite (e.g. `ts-morph` or `sed`). This is advisory â€” it does not block planning but should be noted in the deployment runbook. |

---

# Phase 1 â€” Greenfield Development

Phase 2 must not begin until Phase 1 is fully accepted by the user.

---

## Sub-Task ST-01 â€” Quick Fixes
...
**Tests:**
- Unit: update `api/src/tests/utils/validation.test.ts` â€” add a test for TLD-less email acceptance; confirm existing valid-email tests still pass
- Integration: `api/src/tests/integration/examinations.test.ts` or `audit` test â€” verify `PATIENT_SEARCH` audit record appears after a search call
...
## Sub-Task ST-02 â€” Storage Layer Improvements
...
**Tests:**
- Unit: `api/src/tests/utils/` â€” test that `getTableClient` returns the same instance on second call (mock `TableClient.fromConnectionString`)
- Unit: test that `ensureTableExists` calls `createTable` only once for a given name
- Integration: examination create â†’ update flow; assert update succeeds and response is 200
...
## Sub-Task ST-03 â€” Shared Serializer Utilities
...
2. Move `ExaminationCreateBody` â†’ `ExaminationCreateRequest` and `ExaminationBody` â†’ `ExaminationUpdateRequest` to `api/src/types/index.ts`; add `etag` to `ExaminationUpdateRequest`
...
6. Also fix `responseHelpers.ts`: change `` `req_${Date.now()}_${randomUUID()}` `` to `` `req_${randomUUID()}` `` (Â§4.6)
...
**Tests:**
- Unit: `examinationSerializer.test.ts` â€” round-trip: `deserializeExaminationData(serializeExaminationData(data))` equals `data` for a sample payload with nested objects
- Integration: GET â†’ POST â†’ GET examination flow; verify `data` field is correctly serialized and deserialized
...
## Sub-Task ST-04 â€” Type Model Update

**Intent:** Replace the flat-field type model (biometry/doppler at top level + `ft_*`/`twin2_*` keys in `data`) with the Observable-based fetus array model (Â§14.2) in both `api/src/types/index.ts` and `frontend/src/types/index.ts`.
...
**Todo List:**
1. Add `Observable`, `GaFromBiometry`, `FetusSectionData`, new `ExaminationData` interfaces to `api/src/types/index.ts` per Â§14.2
...
**Tests:**
- Unit: update `validation.test.ts` â€” add test for `examinationType: 'prenatal'` (pass) and `'ultrasound_prenatal'` (fail)
...
## Sub-Task ST-05 â€” Frontend Form State Refactor

**Intent:** Replace the ~250 flat `formData` keys in `useExaminationForm.ts` with the `fetuses: FetusSectionFormData[]` array model using `ObservableFormMap`, `AutoCalcValue<string>`, and `ObservableFormFieldState` (Â§15.2). Implement `buildFetusSectionFormData`, `buildSubmitPayload`, `handleFetusChange`, `handleFetusCountChange`, and the `computeObservableDerivedFields` reactive calculation hook (Â§15.3.2) including the EFW cascade rule (Â§14.5). Replace `useBiometryAutoCalc.ts` and `useFirstTrimesterAutoCalc.ts` with the new single hook.
...
- `fetusCount` is runtime state; on the create form, changing `fetusCount` resizes `formData.fetuses` (increase: append empty; decrease: slice â€” no warning on decrease)
...
**Todo List:**
1. Create `frontend/src/types/formData.ts` with all interfaces from Â§15.2
2. Create `frontend/src/utils/observableRegistry.ts` with `OBSERVABLE_CALC_REGISTRY` (Â§15.3.1) importing existing calculation functions from `calculations.ts`; add `enrichTypeConfig`
3. Implement `computeObservableDerivedFields` in `observableRegistry.ts` per Â§15.3.2, including the EFW cascade rule
4. Implement `buildFetusSectionFormData` in `observableRegistry.ts` or a new `formDataMappers.ts` per Â§15.4.1; handle free-text types (`vp`, `la`) without numeric conversion
5. Implement `buildSubmitPayload` per Â§15.4.2; handle free-text types; omit `isManual: false` from storage
...
**Tests:**
- Unit: `computeObservableDerivedFields` â€” BPD input â†’ BPD GA and percentile auto-calculated; manual BPD GA not overridden; EFW cascade rule for all 3 rules from Â§14.5
- Unit: `buildSubmitPayload` â€” round-trip with a sample `FetusSectionFormData`; free-text `vp`; `isManual: false` omitted from output
- Unit: `buildFetusSectionFormData` â€” seeding from a stored `FetusSectionData` with mixed present/absent observables
- Unit: `handleFetusCountChange(3)` â†’ `formData.fetuses.length === 3`; `handleFetusCountChange(1)` â†’ `formData.fetuses.length === 1`

**Relevant Context:** `frontend/src/hooks/useExaminationForm.ts`, `frontend/src/hooks/useBiometryAutoCalc.ts`, `frontend/src/hooks/useFirstTrimesterAutoCalc.ts`, `frontend/src/utils/calculations.ts`, `docs2/refactoring-findings.md Â§14.5`, `docs2/refactoring-findings.md Â§15.3`, `docs2/refactoring-findings.md Â§15.4`

**Status:** [x] complete

---

## Sub-Task ST-06 â€” `EXAM_TYPE_CONFIG` Implementation

**Intent:** Replace `SECTION_VISIBILITY` (4-entry boolean map) and the standalone `isFirstTrimester`/`isFtTwins` helpers with a 2-entry `EXAM_TYPE_CONFIG` (Â§12.3.3) carrying `label`, `trimester`, `biometryTypes: readonly ObservableTypeConfig[]`, and `dopplerTypes: readonly ObservableTypeConfig[]`. All observable type metadata (label, unit, `hasPercentile`, `hasGa`, `validRange`, `sourceTag`) is defined here.
...
- Full observable type lists for both exam types defined per Â§14.3 (prenatal biometry: `bpd`, `ofd`, `hc`, `ac`, `fl`, `efw`, `tcd`, `tad`, `apad`, `cm`, `nuchalFold`, `nb`, `lc`, `la`, `vp`; prenatal doppler: `pi`, `ri`, `utADexPI`, `utADexRI`, `utASinPI`, `utASinRI`, `cma`, `psv`, `cpr`, `ducVen`; FT biometry: `crl`, `nt`, `nb`, `puls`; FT doppler: `utADexPI`, `utADexRI`, `utASinPI`, `utASinRI`)
...
2. Build the `prenatal` config entry with all biometry and doppler `ObservableTypeConfig` entries per Â§14.3; use `enrichTypeConfig` to add `validRange` and `sourceTag`
3. Build the `first_trimester` config entry with FT biometry and doppler entries per Â§14.3
...
**Tests:**
- Unit: validate `EXAM_TYPE_CONFIG` structure â€” entry counts, required fields present, `enrichTypeConfig` applied (spot-check `bpd.validRange` and `crl.sourceTag`)

**Relevant Context:** `frontend/src/constants/examinationTypes.ts`, `frontend/src/utils/observableRegistry.ts`, `docs2/refactoring-findings.md Â§12.3.3`, `docs2/refactoring-findings.md Â§14.3`

**Status:** [x] complete

---

## Sub-Task ST-07 â€” Config-Driven Form Rendering and Screen Layout

**Intent:** Refactor `ExaminationForm.tsx` to drive all section rendering from `EXAM_TYPE_CONFIG` and `formData.fetuses[]` via new `ObservableSection`/`ObservableRow` components. Replace the twin CSS grid with the horizontal-scroll flex layout (Â§13.3). Add the fetus count selector to the create form. Update `ExaminationDetailPage.tsx` and `ExaminationSections.tsx` to read from `examination.data.fetuses[i]`. Update exam type filter dropdowns to 2 values.

**Expected Outcomes:**
- New files: `frontend/src/components/sections/ObservableRow.tsx`, `frontend/src/components/sections/ObservableSection.tsx` per Â§15.5.1 and Â§15.5.2
...
- Fetus section container uses `display: flex; flex-direction: row; flex-wrap: nowrap; gap: 1.5rem; overflow-x: auto` â€” NOT CSS grid; each fetus column has `min-width: 480px; flex: 0 0 auto`
...
- `ExaminationDetailPage.tsx`: summary tile shows `"Type: Prenatal â€” N fetuses"` composite label; `isTwins` / `isFt` derivations removed; `examination.data.fetuses.length` used for fetus count
...
- List view "Type" column shows `"Prenatal"` or `"First Trimester"` optionally with `" (Ã—N)"` suffix for N > 1 fetuses
...
- **Detail page** (`ExaminationDetailPage.tsx` and `ExaminationSections.tsx`): all `AutoCalcDot` usages (5 sites) replaced by the same `autoSuffix` text-suffix pattern applied to **rendered value cells** â€” `"28w 2d (Hadlock)"`, `"28w 2d (manual)"`, etc.; all footnote legend rows (*"â— Value manually entered"*) removed
...
2. Create `ObservableRow.tsx` per Â§15.5.1 using `PercentileInput` for the percentile column; compose labels using `autoSuffix` â€” `\`${label} (${unit})\`` + `autoSuffix(isManual, sourceTag)` when a value is present; do **not** import `AutoCalcDot` or the old `autoCalcLabel`
3. Create `ObservableSection.tsx` per Â§15.5.2
...
5. Update fetus section container CSS from grid to flex per Â§13.3
...
- AC-ST07-5: Fetus count = 1: no column header "Fetus 1" shown; fetus count â‰¥ 2: column headers "Fetus 1", "Fetus 2", ... shown
...
- AC-ST07-11: Detail page biometry grid shows `"28w 2d (Hadlock)"` for an auto-calculated BPD GA value; shows `"28w 2d (manual)"` in yellow for a manually overridden value; shows `"â€”"` for an absent value with no suffix
- AC-ST07-12: No footnote legend rows (*"â— Value manually entered"*) exist anywhere in the detail page or `ExaminationSections`

**Tests:**
- Unit: `ObservableRow` renders with and without percentile/GA columns based on `config.hasPercentile`/`hasGa`
- Unit: Fetus count change from 1 to 3 â†’ `formData.fetuses.length === 3`; from 3 to 2 â†’ `formData.fetuses.length === 2`
- End-to-end: Create prenatal exam with 2 fetuses â†’ save â†’ load detail â†’ both fetus sections visible; edit â†’ fetus count read-only

**Relevant Context:** `frontend/src/components/ExaminationForm.tsx`, `frontend/src/components/ExaminationSections.tsx`, `frontend/src/pages/ExaminationDetailPage.tsx`, `frontend/src/pages/ExaminationsPage.tsx`, `frontend/src/pages/PatientDetailPage.tsx`, `frontend/src/components/sections/BiometrySection.tsx`, `docs2/refactoring-findings.md Â§13.3`, Â§15.5

**Status:** [x] complete

---

## Sub-Task ST-08 â€” View Model and PDF Refactor

**Intent:** Replace `ExamPdfViewModel` (flat twin/FT optional fields) with a `fetuses: FetusPdfViewModel[]` array; refactor `viewModelBuilders.ts` to map from `exam.data.fetuses[]` to `FetusPdfViewModel[]`; refactor `pdfDocument.ts` and `pdfSections.ts` to use the pair-loop layout (Â§13.4) supporting N fetuses.
...
- `viewModelBuilders.ts`: `buildViewModel` iterates `exam.data.fetuses`; builds `FetusPdfViewModel[]`; header title uses type + fetus count composite (Â§13.5); dagger footnote legend (`\nâ€  Value manually entered`) is always statically appended to notes alongside `PRENATAL_BIOMETRY_CITATIONS` / `FT_BIOMETRY_CITATIONS` without conditional flag scanning; per-field daggers continue via `withManualMarker`
- `pdfDocument.ts`: `buildExaminationPDF` uses the pair-loop (Â§13.4.4); `drawCommonSections`, `drawClinicalInformation`, `drawSignatureLine`, `drawFooter` extracted as named functions; common sections repeated on every page for N > 2 fetuses; footer shows `"Page N of M"`
- `pdfSections.ts`: `renderClinicalSections` (renamed `renderClinicalSectionsPair`) accepts `pair: FetusPdfViewModel[]` and `layout: PairLayout` instead of `isTwins` + hard-coded X constants; `PdfDrawHelpers` loses `TWIN_COL_W`, `T1_X`, `T2_X`; iterates `pair.length`; `chunkFetuses` and `computePairLayout` implemented per Â§13.4.1â€“13.4.2; biometry rows rendered by iterating `fetus.biometry` observables, with label/unit looked up from a static `OBSERVABLE_PDF_META` map keyed by `type`
- All existing PDF content fields continue to appear (parity requirement: medical content unchanged)
- `OBSERVABLE_PDF_META` map covers all observable types from Â§14.3 for both exam types
...
2. Rewrite `buildViewModel` in `viewModelBuilders.ts` to iterate `exam.data.fetuses`; compute header title per Â§13.5; statically append `\nâ€  Value manually entered` to the notes citations string unconditionally (simplifies R-11, no flag scanning required)
3. Add `chunkFetuses` and `computePairLayout` to `pdfSections.ts` per Â§13.4.1â€“13.4.2
4. Add `OBSERVABLE_PDF_META` map to `pdfSections.ts` covering all types from Â§14.3
5. Refactor `renderClinicalSections` â†’ `renderClinicalSectionsPair(doc, vm, pair, layout, y, helpers, pageIdx, totalPages)` per Â§13.4.5; remove `isTwins`/`isFt` branches; iterate `pair.length`
6. Extract `drawCommonSections`, `drawClinicalInformation`, `drawSignatureLine`, `drawFooter` from `buildExaminationPDF` body
7. Replace `buildExaminationPDF` linear body with the pair-loop per Â§13.4.4
...
**Relevant Context:** `frontend/src/services/print.service.ts`, `frontend/src/services/viewModelBuilders.ts`, `frontend/src/components/reports/pdfDocument.ts`, `frontend/src/components/reports/pdfSections.ts`, `docs2/refactoring-findings.md Â§13.4`, Â§13.5

**Status:** [x] complete

---

## Sub-Task ST-09 â€” Migration Script
...
- **Pass 1 â€” Fetus array migration:** For each entity in the `EXAM` and `PATIENT_*` partitions (excluding `MRN` partition), reads legacy `biometry`, `doppler`, `biometry2`, `doppler2`, `gestationalAgeFromBiometry`, `gestationalAgeFromBiometry2`, and all `data.*` fields; constructs `data.fetuses[]` per the new model; removes the legacy top-level fields; rewrites the entity. Handles free-text types (`vp`, `la`) correctly. Handles FT-specific mapping (`data.ft_biometry` â†’ `fetuses[0].biometry` with CRL/NT/NB/Puls types; `data.ft_markers` â†’ `fetuses[0].markers`; `gaFromCrl`/`gaFromBio`/`gaFromCrlIsManual`/`gaFromBioIsManual` â†’ `fetuses[0].gaFromBiometry`). Handles twins (`biometry2` â†’ `fetuses[1].biometry`, etc.). Handles all four legacy exam types.
- **Pass 2 â€” Exam type rewrite:** For each entity in `EXAM` and `PATIENT_*` partitions, rewrites `examinationType` per the mapping in Â§12.5: `ultrasound_prenatal` â†’ `prenatal`, `ultrasound_prenatal_twins` â†’ `prenatal`, `ultrasound_first_trimester` â†’ `first_trimester`, `ultrasound_first_trimester_twins` â†’ `first_trimester`
...
**Tests:**
- Unit: `migratePrenataleEntity(legacyEntity)` â†’ validates output shape for each legacy exam type variant (4 cases)
- Unit: `migrateExamType(entity)` â†’ correct type mapping for all 4 input values
- Integration: run script against Azurite with seeded sample data; verify counts and spot-checks

**Relevant Context:** `api/src/utils/tableClient.ts`, `api/src/types/index.ts`, `docs2/refactoring-findings.md Â§8.1`, Â§12.5, Â§14.3

**Status:** [x] complete

---

## Sub-Task ST-10 â€” Directory Restructuring

**Intent:** Move all files to the proposed target directory structure (Â§5) using `git mv` and an automated import-path update pass. This is the last sub-task â€” all functional work is complete; this is purely organizational.

**Expected Outcomes:**
- Backend: `api/src/functions/` sub-divided into `auth/`, `patients/`, `examinations/`, `users/`, `audit/`, `system/` per Â§5.1
- Backend: `api/src/utils/` reorganized into `api/src/shared/` with sub-directories per Â§5.1; `examinationSerializer.ts` in `shared/storage/`
- Frontend: `frontend/src/pages/` reorganized into `frontend/src/features/` per Â§5.2; page files moved into feature directories
...
4. Move and rename `print.service.ts` â†’ `printService.ts`
5. Move `viewModelBuilders.ts` â†’ `reports/`
...
### Examinations â€” Auto-Calculation
...
### Examinations â€” Detail Page
- [ ] Detail page for a 2-fetus prenatal exam shows 2 fetus data sections
- [ ] Detail page type label shows `"Prenatal â€” 2 fetuses"` for a 2-fetus prenatal exam
...
- [ ] PDF biometric citations appear in notes with the dagger footnote legend (`\nâ€  Value manually entered`) statically included; individual overridden fields display the ` â€ ` marker
...
# Phase 2 â€” Migration Procedures and Scripts
...
3. **Pass 1 â€” Fetus array migration:** Run `node scripts/migrate-v2.ts --pass 1 --connection-string <conn>`. Verify exit code 0.

4. **Pass 1 validation (see Â§2.3 below):** Run the validation checklist. All checks must pass before proceeding to Pass 2.

5. **Pass 2 â€” Exam type rewrite:** Run `node scripts/migrate-v2.ts --pass 2 --connection-string <conn>`. Verify exit code 0.

6. **Pass 2 validation (see Â§2.3 below).**
...
- [ ] `ultrasound_prenatal` â†’ `prenatal` mapping correct
- [ ] `ultrasound_prenatal_twins` â†’ `prenatal` mapping correct
- [ ] `ultrasound_first_trimester` â†’ `first_trimester` mapping correct
- [ ] `ultrasound_first_trimester_twins` â†’ `first_trimester` mapping correct
